...
#include "MBSTFTestObjectStore.hh"

MBSTF_NAMESPACE_START

int MBSTFTestObjectStore::testObjectStore(/*MBSTFObjectStore& store*/) {
    std::string object1 = "object1";
    std::string object2 = "object2";
    const std::string type1 = "type1";
    const std::string type2 = "type2";

    MBSTFObjectStore store;

    // Create some initial data
    MBSTFObjectStore::ObjectData testData1 = {0x31, 0x32};
    MBSTFObjectStore::ObjectData testData2 = {0x50, 0x51, 0x52};
...
void MBSTFTestObjectStore::addObject(MBSTFObjectStore& store, const std::string& object_id, const MBSTFObjectStore::ObjectData& data, const std::string& type) {
    store.addObject(object_id, MBSTFObjectStore::ObjectData(data), type);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
}

void MBSTFTestObjectStore::getObject(MBSTFObjectStore& store, const std::string& object_id) {
    try {
	std::ostringstream retrievedDataElements;
        std::ostringstream retrievedDataSize;
        std::ostringstream rData;

        MBSTFObjectStore::ObjectData retrievedData = store.getObject(object_id);
...
void MBSTFTestObjectStore::getMetadata(MBSTFObjectStore& store, const std::string& object_id) {

    std::ostringstream meta;
   // Get metadata from the object store
    MBSTFObjectStore::Metadata metadata = store.getMetadata(object_id);
...
void MBSTFTestObjectStore::deleteObject(MBSTFObjectStore& store, const std::string& object_id) {
...
std::string MBSTFTestObjectStore::timePointToString(const std::chrono::time_point<std::chrono::system_clock, std::chrono::milliseconds>& tp) {
