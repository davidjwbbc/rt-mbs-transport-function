...
void ObjectStore::addObject(const std::string& object_id, ObjectData&& object, const std::string& mediaType) {
    ...
    Metadata metadata = populateMetadata(mediaType);
    m_store[object_id] = std::make_pair(std::move(object), metadata);
}

const ObjectStore::ObjectData& ObjectStore::getObject(const std::string& object_id) const {
    std::lock_guard<std::recursive_mutex> lock(m_mutex);
...
ObjectStore::Metadata ObjectStore::populateMetadata(const std::string& mediaType) {
    //auto now = std::chrono::system_clock::now();
    auto now = std::chrono::time_point_cast<std::chrono::milliseconds>(std::chrono::system_clock::now());

    return Metadata{ mediaType, now, now };
}
