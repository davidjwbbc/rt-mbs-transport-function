    ...
    ,Subscriber()	
{
    ...
    //auto& service = const_cast<SubscriptionService&>(objectStore());
    /*
    const ObjectStore& store = objectStore();
    const SubscriptionService& service = static_cast<const SubscriptionService&>(store);
    */
    ObjectStore& store = objectStore(); // Non-const access
    SubscriptionService& service = store;
    //SubscriptionService& service = store;
...
std::shared_ptr<ObjectListPackager> &ObjectListController::setObjectListPackager(DistributionSession &distributionSession, const ObjectStore &object_store) {
